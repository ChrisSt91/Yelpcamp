...
const map = new mapboxgl.Map({
	...
	center: [-103.5917, 40.6699], // default center
	zoom: 3,
});

map.on("load", () => {
	// Add campgrounds source
	...
	map.on("click", "unclustered-point", (e) => {
		...
		const { title, location, id } = e.features[0].properties;

		new mapboxgl.Popup()
			.setLngLat(coordinates)
			.setHTML(
				`
                <strong><a href="/campgrounds/${id}">${title}</a></strong><br>
                ${location}
            `
			)
			.addTo(map);
